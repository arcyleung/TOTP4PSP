...
intraFont* intraFontLoad(const char *filename, unsigned int options) {
  ...
  if (!font) {
    pspDebugScreenPrintf("MALLOC FAILED ");
    return NULL;
  }
  ...
  if (!file) {
    pspDebugScreenPrintf("FOPEN FAILED %s", filename);
    return NULL;
  }
  ...
  if (fread(&header, sizeof(PGF_Header), 1, file) != 1) {
    fclose(file);
    pspDebugScreenPrintf("FREAD PGF FAILED ");
    return NULL;
  }
  ...
  if ((strncmp(header.pgf_id,"PGF0",4) == 0) && (header.version == 6) && (header.revision == 2 || header.revision == 3) &&
    ...
    pspDebugScreenPrintf("PGF VALIDATE FAILED ");
      fclose(file);
    return NULL;
  }
  ...
  if (font->fileType == FILETYPE_PGF) {
    ...
    if (!font->glyphBW) {
      fclose(file);
      intraFontUnload(font);
      pspDebugScreenPrintf("glyphbw FAILED ");
      return NULL;
    }  
  ...
  if (!font->filename || !font->texture) {
    fclose(file);
    intraFontUnload(font);
    pspDebugScreenPrintf("filename || texture FAILED ");
    return NULL;
  }
  ...
  if (font->fileType == FILETYPE_PGF) {
    ...
    if (!advancemap) {
      fclose(file);
      intraFontUnload(font);
      pspDebugScreenPrintf("advancemap FAILED ");
      return NULL;
    }  
    if (fread(advancemap, header.advance_len*sizeof(signed long)*2, 1, file) != 1) {
      ...
      fclose(file);
      intraFontUnload(font);
      pspDebugScreenPrintf("advancemap2 FAILED ");
      return NULL;
    }  
    ...
    if (ucs_shadowmap == NULL) {
      ...
      fclose(file);
      intraFontUnload(font);
      pspDebugScreenPrintf("shadowmap FAILED ");
      return NULL;
    }
    ...
    if (header.revision == 3) {
      if (fread(font->charmap_compr, font->charmap_compr_len*sizeof(unsigned short)*2, 1, file) != 1) {
        ...
        fclose(file);
        intraFontUnload(font);
        pspDebugScreenPrintf("6.3 charmap FAILED ");
        return NULL;
      }
    } else {
    ...
    if (header.charmap_bpe == 16) { //read directly from file...
      if (fread(font->charmap, header.charmap_len*sizeof(unsigned short), 1, file) != 1) {
        ...
        fclose(file);
        intraFontUnload(font);
        pspDebugScreenPrintf("6.3 charmap 2 FAILED ");
        return NULL;
      }
    } else {
      ...
      if (id_charmap == NULL) {
        ...
        fclose(file);
        intraFontUnload(font);
        pspDebugScreenPrintf("id charmap FAILED ");
        return NULL;
      }
    ...
    if (charptr == NULL) {
      ...
      fclose(file);
      intraFontUnload(font);
      pspDebugScreenPrintf("charptr FAILED ");
      return NULL;
    }
    ...
    if (font->fontdata == NULL) {
      ...
      fclose(file);
      intraFontUnload(font);
      pspDebugScreenPrintf("fontdata FAILED ");
      return NULL;
    }
    if (fread(font->fontdata, len_fontdata*sizeof(unsigned char), 1, file) != 1) {
      ...
      fclose(file);
      intraFontUnload(font);
      pspDebugScreenPrintf("fontdata 2 FAILED ");
      return NULL;
    }
    ...
    if ((options & PGF_CACHE_MASK) == INTRAFONT_CACHE_ASCII) {
      ...
      if (font->n_chars == 0) { //no ASCII chars in fontfile -> abort
        ...
        free(charptr);
        intraFontUnload(font);
         pspDebugScreenPrintf("no ascii FAILED ");
        return NULL;
      }
    ...
    for (i = 0; i < font->n_chars; i++) {
      ...
      if (!intraFontGetGlyph(font->fontdata, &j, PGF_CHARGLYPH, advancemap, &(font->glyph[i]))) {
        ...
        free(charptr);
        intraFontUnload(font);
         pspDebugScreenPrintf("getglyph FAILED ");
        return NULL;
      }
    ...
    for (i = 0; i < font->n_chars; i++) {
      ...
      if (char_id < font->n_chars && font->shadowGlyph[shadow_id].shadowID == 0) { //valid char and shadow glyph not yet loaded
        ...
        if (!intraFontGetGlyph(font->fontdata, &j, PGF_SHADOWGLYPH, NULL, &(font->shadowGlyph[shadow_id]))) {
          ...
          free(charptr);
          intraFontUnload(font);
           pspDebugScreenPrintf("getglyph 2 FAILED ");
          return NULL;
        }      
    ...
    if ( (options & INTRAFONT_CACHE_ASCII) && ((y + ysize + 1) <= font->texHeight) ) { //cache all and does it fit into cache?
      if (!intraFontPreCache(font,options)) {
        intraFontUnload(font);
         pspDebugScreenPrintf("precache FAILED ");
        return NULL;      
      }
    }
    ...
    if (font->fontdata == NULL) {
      fclose(file);
      intraFontUnload(font);
       pspDebugScreenPrintf("bwfon fontdata FAILED ");
      return NULL;
    }
    if (fread(font->fontdata, filesize*sizeof(unsigned char), 1, file) != 1) {
      fclose(file);
      intraFontUnload(font);
      pspDebugScreenPrintf("bwfon fontdata 2 FAILED ");
      return NULL;
    }
    ...
    if ((options & PGF_CACHE_MASK) == INTRAFONT_CACHE_ASCII) {
      intraFontUnload(font);
      pspDebugScreenPrintf("bwfon ascii chars FAILED ");
      return NULL;
    }  
